...
// // const initialSuggestions = ['Tech Support', 'Sales'];
...
// //       this.logger.log(`Escalation email sent successfully to ${recipientEmails.join(', ')} for session ${sessionId}`);
// //     } catch (e) {
// //       this.logger.error(`Failed to send escalation email: ${e.message}`);
// //     }
// //   }

// //   private getNextQuestionAndSuggestions(state: SessionState): { nextQuestion: string | null; suggestions: string[] } {
...
// //       return {
// //         nextQuestion:
// //           'Hello, and welcome to JUSTmobile Customer Service!\n\nMy name is Bele. How can I help you today?\n\nAre you looking for Tech Support or Sales assistance?',
...
// //   private async askGrok(sessionId: string, userInput: string): Promise<{ reply: string; suggestions: string[] }> {
// //     if (!this.conversationData[sessionId]) {
// //       this.logger.error(`Invalid session_id: ${sessionId}`);
// //       return { reply: 'Session not found. Please start a new conversation.', suggestions: [] };
// //     }
...
// //     if (userInput) {
// //       this.conversationData[sessionId].history.push({ role: 'user', content: userInput });
// //       if (state.mode === 'initial') {
// //         const normalizedInput = userInput.toLowerCase().trim();
// //         const validTypes = initialSuggestions.map((t) => t.toLowerCase());
// //         const index = validTypes.indexOf(normalizedInput);
// //         if (index !== -1) {
// //           state.type = initialSuggestions[index];
// //           state.mode = 'conversational';
// //         } else {
// //           state.error = 'Please select one of the following: Tech Support or Sales.';
// //           return { reply: state.error, suggestions: [] };
// //         }
// //       }
// //     }

// //     const { nextQuestion, suggestions } = this.getNextQuestionAndSuggestions(state);
...
// // Use the following knowledge base to answer queries: ${KNOWLEDGE_BASE}
// // Always start with empathy if issue-related, provide step-by-step guidance, suggest upsells naturally.
// // Keep responses concise, end with “Is there anything else I can help with?”
// // Ask this question or provide this information: "${nextQuestion}"
// // `,
// //       };
// //     } else {
...
// //         state.type === 'Tech Support'
// //           ? 'technical troubleshooting, connectivity, device issues, billing errors, and account management'
// //           : 'plans, promotions, add-ons, switching providers, and sales inquiries';
// //       systemPrompt = {
// //         role: 'system',
// //         content: `
// // You are Bele, an empathetic and efficient AI customer support assistant for JUSTmobile, specialized in ${state.type}.
// // Use the following knowledge base to answer queries accurately: ${KNOWLEDGE_BASE}
// // Prioritize empathy (e.g., “I’m sorry you’re experiencing this”), step-by-step guidance, natural upsells.
...
// //         model: 'grok-2-latest',
// //         messages: [systemPrompt, ...this.conversationData[sessionId].history.slice(-15)],
// //       });
// //       const reply = response.choices[0].message.content!;
// //       this.conversationData[sessionId].history.push({ role: 'assistant', content: reply });
// //       this.conversationData[sessionId].history = this.conversationData[sessionId].history.slice(-15);
// //       return { reply, suggestions };
// //     } catch (e) {
// //       this.logger.error(`Error calling Grok API: ${e.message}`);
// //       const fallbackReply = 'Sorry, something went wrong while processing your request. Please try again or contact support.';
// //       this.conversationData[sessionId].history.push({ role: 'assistant', content: fallbackReply });
// //       this.conversationData[sessionId].history = this.conversationData[sessionId].history.slice(-15);
// //       return { reply: fallbackReply, suggestions };
// //     }
// //   }

// //   async processQuery(userId: string, request: QueryRequestDto): Promise<{ message: string; session_id: string; suggestions: string[] }> {
// //     let sessionId = request.session_id;
// //     if (sessionId) {
// //       if (!this.conversationData[sessionId] || this.conversationData[sessionId].state.userId !== userId) {
// //         throw new HttpException('Invalid or unauthorized session ID', HttpStatus.UNAUTHORIZED);
// //       }
// //     } else {
...
// //   async processEscalation(userId: string, request: EscalationRequestDto): Promise<string> {
...
// //       throw new HttpException('Invalid email format. Please provide a valid email address.', HttpStatus.BAD_REQUEST);
// //     }
...
// //     const confirmation = "Your issue has been escalated to a live agent. We'll contact you soon!";
// //     this.conversationData[sessionId].history.push({ role: 'assistant', content: confirmation });
// //     this.conversationData[sessionId].history = this.conversationData[sessionId].history.slice(-15);

// //     await this.sendEscalationEmail(sessionId, userId, request.full_name, request.email, request.phone, request.issue_description);
