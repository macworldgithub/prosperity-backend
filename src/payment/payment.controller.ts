...
import { AddPaymentMethodDto } from './dto/add-payment-method.dto';
import { MakePaymentDto } from './dto/make-payment.dto';
import { ProcessRecurringPaymentDto } from './dto/process-recurring-payment.dto';
import {
  ApiTags,
  ApiOperation,
  ApiResponse,
...
export class PaymentController {
  ...
  @HttpCode(200)
  @ApiOperation({
    summary: 'Process a recurring payment using Quickstream single-use token',
  })
  @ApiBody({ type: ProcessRecurringPaymentDto })
  @ApiOkResponse({
    description: 'Recurring payment processed successfully',
    schema: {
      example: {
        return: { transactionId: 'txn_123456', status: 'approved' },
  ...
  async processRecurringPayment(@Body() dto: ProcessRecurringPaymentDto) {
    const result = await this.paymentService.processRecurringPayment(dto);
    return formatResponse(result, 'Recurring payment processed successfully');
  }
}
